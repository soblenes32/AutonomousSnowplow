package com.asl.snowplow.rplidar;
...
public class RpLidarLowLevelDriver {
	...
	protected void sendNoPayLoad(byte command) {
		if (verbose) {
			System.out.printf("Sending command \n", command & 0xFF);
		}
...
//						System.out.println("--- Bad Packet ---");
				...
				if (start0 == SYNC_BYTE0 && start1 == SYNC_BYTE1) {
					...
					if (parsePacket(data, offset + 2 + 4 + 1, packetLength, dataType)) {
						...
						System.out.println("lastReceived set to: "+lastReceived);
						offset += 2 + 5 + packetLength;
					} else {
						System.out.println("NOT setting lastReceived");
						offset += 2;
					}
				} else {
					offset++;
	...
	protected boolean parsePacket(byte[] data, int offset, int length, byte type) {
		switch (type) {
			...
			case (byte) RCV_HEALTH: // health
				boolean healthComplete = parseHealth(data, offset, length);
				health.print();
	...
	protected boolean parseScan(byte[] data, int offset, int length) {
		...
		byte b0 = data[offset]; //Should be A5
		byte b1 = data[offset + 1]; //Should be 5A
	...
	*/
	...
	public class ReadSerialThread implements Runnable {
		...
		public void run() {
			...
			System.out.println("RPLidar thread is terminating...");
		}
	}
}
